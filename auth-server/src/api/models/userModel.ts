...
const getUserById = async (user_id: number): Promise<UserWithProfilePicture> => {
  const [rows] = await promisePool.execute<RowDataPacket[] & UserWithProfilePicture[]>(
...
const createUser = async (
  ...
  const stmt = promisePool.format(sql, [user.username, user.email, user.password, user_level_id]);
